...
def convert_list_to_grid(boardList, boardSize = 4):
    boardList = np.array(boardList)
    boardList = boardList.reshape((boardSize,boardSize))
    boardDict = {}
    for i in range(boardSize):
        for j in range(boardSize):
            boardDict[(i,j)] = boardList[i][j]
    return boardDict
...
def SolveBoard(board):
    ...
    global stems
    global paths
    grid = convert_list_to_grid(board)
