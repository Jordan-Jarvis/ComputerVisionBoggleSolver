...
def show_digits(digits, scale, colour=255):
    """Shows list of 81 extracted digits in a grid format"""
    rows = []
    with_border = [cv2.copyMakeBorder(img.reshape(math.ceil(250/scale),math.ceil(250/scale),), 1, 1, 1, 1, cv2.BORDER_CONSTANT, None, colour) for img in digits]
    for i in range(scale):
        row = np.concatenate(with_border[i * scale:((i + 1) * scale)], axis=1)
        rows.append(row)
...
#    cv2.imshow('img', img)
    for square in squares:
        digits.append(extract_digit(img, square, size).flatten())
    return digits


def parse_grid(original, scale):
...
#    cv2.imshow('processed', processed_img)
    ...
    squares = infer_grid(cropped, scale)
    digits = get_digits(cropped, squares, math.ceil(250/scale))
    #for i in range(len(digits)):
        #cv2.imwrite("trainingData//square " + str(i) + " g.png", digits[i])
    final_image = show_digits(digits, scale)
    return final_image, cropped, digits
	
def extract_boggle(image):
	final_image = parse_grid(image, 4)
	return final_image

def extract_sudoku(image):
    final_image = parse_grid(image, 9)
    return final_image